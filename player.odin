...
player_draw_reload_bar :: proc(player: Player){
	...
	rl.DrawRectangleRec(rl.Rectangle{outer_rec.x + offset, outer_rec.y + offset, (outer_rec.width - 2 * offset) * fill_amount, outer_rec.height - 2 * offset}, rl.Color{224, 224, 224, 255})
...
player_draw_health_bar :: proc(player: Player){
	outer_rec := rl.Rectangle{f32(WIDTH) - 50.0 - 140.0, 50.0, 140.0, 30.0}
	offset: f32 = 5.0
	fill_amount := player.health / player.max_health
	rl.DrawRectangleRec(outer_rec, rl.WHITE)
	rl.DrawRectangleRec(rl.Rectangle{outer_rec.x + offset, outer_rec.y + offset, (outer_rec.width - 2 * offset) * fill_amount, outer_rec.height - 2 * offset}, rl.RED)
	rl.DrawTextureEx(player.heart_icon, rl.Vector2{outer_rec.x + outer_rec.width - 24.0, outer_rec.y - 26.0}, 0.0, 4.0, rl.WHITE)
}
